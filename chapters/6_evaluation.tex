...
distributed setting. Its programming model is designed in such a way that the
pipelines that a user creates can be parallelized ad infinitum. However, since
...
% What is the pregel API 
\smallskip
This library provides enables us to specify vertex centric computations which
are performed on the entire graph. The idea behind the vertex centric
computation is to `think like a vertex'. This means that we specify a
computation that a vertex needs to perform based on its neighbours and at the
end of its computation, it has the capability to send some information to its
neighbours. Vertex centric computations run for multiple iterations and in each
iteration, every vertex runs some function based on the input from its
neighbours and optionally gives an output to its neighbours. These vertex
...
we start by labelling our starting node `1' and label the rest of the nodes
...
node is $n$. This way, for each iteration, we label the neighbouring nodes of
the nodes which we explored in the previous iteration. After the desired number
of iterations, we can read the node identifiers of all the nodes whose label
is $h$, where $h$ is the number of hops.

% Comparison with our solution and why is it slow 
% Can it be faster? For which cases is it faster
Figure \ref{fig:flinkCmp} shows a comparison between our solution and Apache
Flink for finding n-hop neighbours of a starting vertex in the SF-1 dataset. 
It is clear that Apache
Flink is around three orders of magnitude slower than our solution. This is
primarily because a Flink cluster needs to load in the data before it can start
performing any computations. Furthermore, the vertex centric computations are
very expensive if there are only a handful of vertices that perform any useful
computation. In this case, we have 3 million vertices whereas there may only be
...
of the graph is required in the result.
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.5\textwidth]{figures/charts/flink_cmp}
...
\bigskip
In this section, we presented the performance and architectural constraints of
some other state of the art systems which may be used to perform traversals on
large graphs. With our findings, we can place our tool within the wider
landscape in terms its use-cases. First, while comparing to Neo4j, we saw that
Neo4j is clearly superior to our solution, in terms of performance, for graphs
...
units. In such cases, our solution may provide a simpler way to query these
graphs. On the other hand, Flink inherently relies on distribution of data among
various computational units. Thus, flink, and other big data frameworks, are
useful when you need to touch the entire graph in order to get the results for
...
you only require part of the graph to answer your query because of the overhead
of reading data from storage, and the vertex centric computation API. This is 
the use-case where our solution is more efficient compared to flink. We provide
